...
app.get('/search', validateRequest, async (req, res) => {
    try {
        ...
        const result = await pool.query('SELECT * FROM data where name like $1' ,[`%${input}%`]);
        res.json(result.rows);
...
app.post('/api/login', async (req, res) => {
    try {
        ...
        // Determine role
        const role = (email === process.env.ADMIN_EMAIL || email === process.env.ADMIN_EMAIL2) ? "admin" : "user";
