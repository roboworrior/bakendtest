...
const validateRequest = (req, res, next) => {
  ...
  jwt.verify(token, JWT_SECRET, (err, user) => {
    ...
    req.user = user; // `user` is the decoded payload
    next();
  });
};
...
app.post("/api/submit", async (req, res) => {
  try {
    ...
    const result = await pool.query(
      "INSERT INTO userinfo(name,phone_number,cart) VALUES ($1, $2 ,$3) RETURNING *",
      [name, phone_number, JSON.stringify(cartItems)]
    );
...
app.post("/api/login", async (req, res) => {
  try {
    ...
    const user = await pool.query("SELECT * FROM logindata WHERE email = $1", [
      email,
    ]);
...
app.post("/api/registor", async (req, res) => {
  try {
    ...
    const result = await pool.query(
      "INSERT INTO logindata(username,password,email) VALUES ($1, $2,$3) RETURNING *",
